...
// ── Die belegte Quelle: das Feld `lo` im Kader ──────────────────────
//
// Nachgewiesen an echten Daten (Diagnoseseite, 29.08.):
// `/v4/leagues/{id}/managers/{uid}/squad` liefert je Spieler ein Feld
// `lo` — die Position in der Aufstellung, **null-basiert**. Wer auf der
// Bank sitzt, hat das Feld gar nicht.
//
// Beispiel eines echten Kaders (14 Spieler, 10 aufgestellt):
//
...
//
// Damit muss nichts mehr geraten werden: Ein Spieler mit `lo` ist
// aufgestellt, einer ohne nicht. Das deckt auch die Fälle ab, an denen
// jede Erkennung über „genau elf" scheitern musste — sieben Aufgestellte,
// zehn, gar keiner.
export function startelfAus(rohListe) {
  const liste = Array.isArray(rohListe) ? rohListe : [];
  if (liste.length === 0) return null;

  const drin = liste.map((s) => {
    const lo = Number(s?.lo);
    // Höchstens elf Positionen, null-basiert. Alles andere ist kein Platz
    // in der Aufstellung.
    return Number.isInteger(lo) && lo >= 0 && lo < ELF;
  });

  return drin.some(Boolean) ? drin : null;
}

// Spieler-IDs der Startelf aus einer Kaderantwort.
export function startelfIds(rohListe) {
  const drin = startelfAus(rohListe);
  if (!drin) return null;
  const ids = new Set();
  rohListe.forEach((s, i) => {
    const id = String(s?.pi ?? s?.i ?? s?.id ?? "");
    if (id && drin[i]) ids.add(id);
  });
...
// Alle Feldnamen, die irgendwo in einer Antwort vorkommen — samt Pfad.
//
// Für die Suche nach der Aufstellung: Wenn `elfAus` nichts findet, hilft
// die Frage „steht überhaupt irgendwo etwas, das nach Aufstellung
// aussieht?" mehr als ein weiterer Rateversuch.
const VERDAECHTIG = /^(lo|lp|lst|st|sta|mdst|lineup|lineup_?order|lineup_?status|lineup_?position|start|starting|inlineup|ins|pos|formation|day|md)$/i;

export function schluesselBaum(daten, tiefe = 0, pfad = "") {
  if (tiefe > 4 || daten == null || typeof daten !== "object") return [];
  if (Array.isArray(daten)) {
    return daten.length ? schluesselBaum(daten[0], tiefe + 1, `${pfad}[]`) : [];
  }
  const raus = [];
  for (const [k, v] of Object.entries(daten)) {
    const voll = pfad ? `${pfad}.${k}` : k;
    const einfach = v == null || typeof v !== "object";
    raus.push({
      pfad: voll,
      wert: einfach ? String(v).slice(0, 20) : Array.isArray(v) ? `[${v.length}]` : "{…}",
      verdaechtig: VERDAECHTIG.test(k),
    });
    if (!einfach) raus.push(...schluesselBaum(v, tiefe + 1, voll));
  }
  return raus;
}
