...
import { erlaubtesMinus } from "./gebot";
import { holeMitspieler } from "./mitspieler";
import { geheimeZugaenge } from "./zugang";
import { wenVerbergen, verbergeKonten } from "./privatsphaere";
...
export async function baueSchnappschuss(leagueId, token, nutzer, ranking) {
  ...
  // **Auch hier.** Was in der Tabelle verborgen ist, darf nicht über den
  // Umweg einer Frage herausfallen — sonst wäre das Schloss ein Witz:
  // „Wie viel Geld hat W1zco?" und das Modell sagt es.
  const ich = manager.find((m) => String(m.i) === String(nutzer) || m.n === nutzer);
  const geheim = wenVerbergen({
    zugaenge: await geheimeZugaenge(),
    spieler: manager,
    betrachterId: ich?.i ?? null,
  });
  const konten = verbergeKonten(
    await berechneKonten(leagueId, manager, settings), geheim);
  ...
  for (const k of konten) {
    ...
    // Gesagt wird, DASS die Zahlen fehlen — nicht nichts. Sonst schätzt
    // das Modell den Kontostand aus dem Rest zusammen und gibt eine
    // erfundene Zahl aus, was schlimmer wäre als eine offene.
    if (k.finanzenGeheim) {
      zeilen.push(
        `- ${k.name} (ID ${k.id}): Finanzzahlen nicht verfügbar — diese Person ` +
        `zeigt sie nicht. NICHT schätzen, NICHT herleiten, auf Nachfrage sagen, ` +
        `dass sie nicht öffentlich sind. Teamwert ${euro(teamwert)}, ` +
        `${eigene.length} Spieler, ${k.punkte} Punkte`
      );
      continue;
    }
