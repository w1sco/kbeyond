...
export async function speichereMarkt(leagueId, token) {
  const daten = await kbFetch(`/v4/leagues/${leagueId}/market`, token);
  const roheListe = daten.it ?? daten.items ?? daten.players ?? findeSpielerListe(daten) ?? [];

  // Startelf-Chance nebenbei mitnehmen, falls das Angebot sie trägt.
  // Kostet keinen Aufruf — und trifft genau die Spieler, bei denen man
  // gerade überlegt zu kaufen.
  const chancen = ernte(roheListe);
  if (chancen.length > 0) await speichereChancen(chancen, await naechsterSpieltag());

  const liste = roheListe
