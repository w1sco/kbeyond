...
import { tageSeit } from "./format";
// Weiterreichen, damit bestehende Importe aus ledger.js weiter gelten.
export { loginBonus, tagesBonus, kommendeLoginBoni, SPIELTAGE, spieltagWahl } from "./loginbonus";
import { sql } from "./db";
...
export async function berechneKonten(leagueId, manager, settings) {
  ...
  // Gutgeschrieben wird um 0:00 Uhr für den neuen Tag. Gezählt werden
  // deshalb Mitternachte in deutscher Zeit, nicht verstrichene 24 Stunden.
  const tage = tageSeit(referenz);
  ...
  // Korrekturen gehören dem Nutzer, nicht der Liga. Welcher gemeint ist,
  // steht in den Einstellungen, mit denen gerechnet wird.
  const korrekturen = new Map(
    (await sql`
      SELECT manager, betrag FROM korrektur
      WHERE league_id = ${leagueId} AND user_id = ${settings.user_id ?? ""}`)
