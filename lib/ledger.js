...
export async function berechneKonten(leagueId, manager, settings, meinName) {
  ...
  const init = (name) => {
    if (!konten.has(name)) konten.set(name, { kaeufe: 0, verkaeufe: 0, anzKauf: 0, anzVerkauf: 0 });
  ...
  // Echte Bonus-Gutschriften seit Stichtag (nur für den eingeloggten Nutzer sichtbar)
  const bonusZeilen = await sql`
  ...
  const bonusEcht = Number(bonusZeilen[0]?.summe ?? 0);
  const bonusTage = bonusZeilen[0]?.anzahl ?? 0;
  const maxTag = bonusZeilen[0]?.max_tag ?? null;

  // Formel als Fallback für Gegner
  const tageFormel = Math.max(0, Math.floor((Date.now() - new Date(stichtag)) / 86_400_000));
  const bonusFormel = settings.login_aktiv ? loginBonus(tageFormel) : 0;
  ...
  return manager.map((m) => {
    ...
    const binIch = meinName && m.n === meinName;
    const bonus = binIch && bonusEcht > 0 ? bonusEcht : bonusFormel;
    ...
    return {
      ...
      loginBonus: bonus,
      bonusIstEcht: Boolean(binIch && bonusEcht > 0),
      punkteBonus,
      korrektur,
      konto: startbudget + bonus + punkteBonus + k.verkaeufe - k.kaeufe + korrektur,
