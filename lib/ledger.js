...
export async function berechneKonten(leagueId, manager, settings, meinName) {
  ...
  // Bonus-Events zur Diagnose und zur Ableitung des Streak-Beginns
  const bz = await sql`
    SELECT COALESCE(SUM((raw->>'bn')::bigint), 0)::bigint AS summe,
           COUNT(*)::int AS anzahl,
           MAX((raw->>'day')::int) AS max_tag,
           MAX(dt) AS letztes_dt
    FROM events
  ...
  // Kontoweiter Streak-Beginn: letztes Bonus-Datum minus (Streak-Tag - 1)
  let streakStart = null;
  if (maxTag && bz[0]?.letztes_dt) {
    streakStart = new Date(new Date(bz[0].letztes_dt).getTime() - (maxTag - 1) * 86_400_000);
    streakStart.setUTCHours(0, 0, 0, 0);
  }

  // Maßgeblich ist der spätere Zeitpunkt: Liga-Reset oder Streak-Beginn
  const stich = new Date(stichtag);
  let referenz = stich;
  let quelle = "Stichtag";
  if (settings.login_start) {
    referenz = new Date(settings.login_start);
    quelle = "manuell gesetzt";
  } else if (streakStart && streakStart > stich) {
    referenz = streakStart;
    quelle = "Streak-Beginn";
  }
