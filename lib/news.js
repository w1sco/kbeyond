...
//
// Zugeordnet wird über eine **laufende Nummer**, nicht über die
// Kickbase-ID. Die ID kennt das Internet nicht — sie im Prompt zu führen
// stiftet nur Verwirrung und taucht im schlimmsten Fall in Suchanfragen
// auf. Gesucht wird über **Name und Verein**, zurück kommt die Nummer.
...
"nr" ist nur unsere interne Nummer aus der Liste — sie taugt nicht als
Suchbegriff. Gesucht wird über Spielername und Verein.
...
du nichts gefunden hast, und gib dazu die **Nummer** aus der Liste an.
...
export function saubereMeldung(roh, nachNummer) {
  // Zugeordnet wird über die laufende Nummer. Ältere Antworten nennen
  // vielleicht noch "id" – beides wird akzeptiert, solange es auf einen
  // Spieler dieses Bündels zeigt.
  const nr = Number(roh?.nr ?? roh?.id);
  const treffer = Number.isFinite(nr) ? nachNummer.get(nr) : null;
  if (!treffer) return null;
  const id = String(treffer.id);
...
export async function holeNews({ schluessel, modell, spieler, modus = "sammeln" }) {
  ...
  // Nummer, Name, Verein — keine Kickbase-ID. Ohne Verein bleibt die
  // Klammer weg, statt eine Team-Nummer als "Verein" auszugeben.
  const nachNummer = new Map(spieler.map((s, i) => [i + 1, s]));
  const liste = spieler
    .map((s, i) => `${i + 1}. ${s.name}${s.verein ? ` — ${s.verein}` : ""}`)
    .join("\n");
  ...
  let verworfen = 0;
  for (const r of roh) {
    const m = saubereMeldung(r, nachNummer);
