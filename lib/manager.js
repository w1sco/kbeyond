...
export function nurMitspieler(liste, aktiv = null, modus = "auto") {
  const alle = liste ?? [];
  if (modus === "immer") return alle;

  const ids = aktiv?.ids ?? (aktiv instanceof Set ? aktiv : null);
  const namen = aktiv?.namen ?? null;

  // Wie oft kommt ein Anzeigename in der Liga vor?
  //
  // Der Feed führt Manager **nur über den Namen**. Gibt es den Namen
  // zweimal — etwa einen mitspielenden Fabinho und einen Admin gleichen
  // Namens —, dann erbt der Admin die Transfers des anderen und taucht
  // fälschlich als Manager auf, samt dessen Kontostand. Bei einem
  // doppelten Namen ist das Feld also kein Beleg, sondern eine
  // Verwechslung; dann zählt nur, was eindeutig an der ID hängt.
  const anzahl = new Map();
  for (const m of alle) anzahl.set(m?.n, (anzahl.get(m?.n) ?? 0) + 1);

  return alle.filter((m) => {
    if (m?.adm !== true) return true;
    if (modus === "nie") return false;
    if (spieltMit(m)) return true;
    if (ids?.has?.(String(m?.i))) return true;
    return (namen?.has?.(m?.n) ?? false) && anzahl.get(m?.n) === 1;
  });
}
