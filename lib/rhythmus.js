...
export function schaetzeZyklus(abstaende, opt = {}) {
  ...
  if (abstaende.length < MINDEST_ABSTAENDE) {
    return { tage: null, grund: "zu wenige Abstände", anzahl: abstaende.length, verworfen: 0 };
  }

  const grenze = new Date(jetzt.getTime() - jungeTage * TAG_MS);
  const jung = abstaende.filter((a) => a.bis >= grenze);
  const basis = jung.length >= MINDEST_ABSTAENDE ? jung : abstaende;
