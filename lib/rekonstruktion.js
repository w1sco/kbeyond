  ...
  for (const tid of teams) {
    try {
      ...
      for (const s of daten.it ?? []) {
        pool.set(String(s.i), { id: String(s.i), name: s.n, quelle: "kader" });
      }
    } catch {
...
export async function rekonstruiere(leagueId, token, stichtag, opt = {}) {
  ...
  if (cache[0]?.daten?.spieler && Date.now() - new Date(cache[0].daten.stand) < 86_400_000) {
    kaderPool = cache[0].daten.spieler;
  } else {
    kaderPool = await holeSpielerPool(token);
    const inhalt = JSON.stringify({ stand: new Date(), spieler: kaderPool });
    await sql`
