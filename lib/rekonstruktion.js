...
//
// ── Wer die Liga verlässt, fliegt raus — aber nur nach einem vollen Lauf ──
//
// Die Kehrseite des Zusammenführens: Ein Spieler, der ins Ausland wechselt
// oder absteigt, stand für immer im Pool und damit als „frei" auf der
// Marktseite — es gab schlicht nichts, das ihn je wieder entfernt hätte.
//
// Entfernt wird deshalb, wer in **keinem** Vereinskader mehr auftaucht.
// Und zwar **ausschließlich nach einem vollständigen Durchlauf**: Fehlt
// auch nur ein Verein, könnte der Spieler dort stehen, und er bliebe. Das
// ist derselbe Schutz, wegen dem überhaupt zusammengeführt wird — ein
// Vereinsabruf, der scheitert, darf keine 25 Spieler kosten.
export async function aktualisierePool(token, opt = {}) {
  ...
  const chancen = [];
  // Wen dieser Lauf wirklich gesehen hat — die Grundlage für den Abgang.
  ...
  // Abgänge: nur nach einem vollen Lauf, siehe oben. Namentlich, damit man
  // sieht, dass es greift — und dass es die Richtigen trifft.
  const abgaenge = [];
  let entfernt = 0;
  if (vollstaendig) {
    for (const [id, s] of nachId) {
      if (gesehen.has(id)) continue;
      nachId.delete(id);
      entfernt++;
      if (abgaenge.length < 5) abgaenge.push(s.name);
    }
  }
