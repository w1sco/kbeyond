...
import { findeSpielerListe, normalisiereSpieler, mwTag, findeAufstellung } from "./format";
...
export async function ladeKader(leagueId, managerIds, token, opt = {}) {
  ...
  for (const uid of managerIds) {
    ...
    if (Date.now() > frist) {
      return { geladen, spieler, leer, ohneNamen, fehler, mitAufstellung, gesamt: managerIds.length, gestoppt: true };
    }
    ...
    try {
      ...
      const startelf = findeAufstellung(roheListe);
  ...
  return { geladen, spieler, leer, ohneNamen, fehler, mitAufstellung, gesamt: managerIds.length, gestoppt: false };
}
