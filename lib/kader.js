  ...
  for (const uid of managerIds) {
    ...
    try {
      ...
      if (liste.length === 0) {
        ...
        await sql`
          INSERT INTO kader (league_id, manager_id, player_id, name, position, marktwert, kaufpreis, punkte, schnitt, aufgestellt, stand)
          SELECT ${leagueId}::text, ${String(uid)}::text, *, NOW() FROM UNNEST(
            ...
            ${liste.map((s) => (s.schnitt == null ? null : Number(s.schnitt)))}::numeric[],
