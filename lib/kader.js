...
export async function ladeKader(leagueId, managerIds, token, opt = {}) {
  ...
  for (const uid of managerIds) {
    ...
    if (Date.now() > frist) {
      return { geladen, spieler, leer, ohneNamen, fehler, mitAufstellung, erkanntAn, leereManager, gesamt: managerIds.length, gestoppt: true };
    }
  ...
  return { geladen, spieler, leer, ohneNamen, fehler, mitAufstellung, erkanntAn, leereManager, gesamt: managerIds.length, gestoppt: false };
}
