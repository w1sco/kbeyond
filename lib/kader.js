...
export async function ladeKader(leagueId, managerIds, token, opt = {}) {
  ...
  for (const uid of managerIds) {
    ...
    if (Date.now() > frist) {
      return { geladen, spieler, leer, ohneNamen, gesamt: managerIds.length, gestoppt: true };
    }
  ...
  return { geladen, spieler, leer, ohneNamen, gesamt: managerIds.length, gestoppt: false };
}
