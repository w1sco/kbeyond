...
// Wochentag in deutscher Zeit: 0 = Sonntag, 6 = Samstag.
//
// Nicht getDay() des Servers nehmen — auf Vercel läuft der in UTC, und
// zwischen 0:00 und 2:00 deutscher Zeit ist dort noch der Vortag.
export function wochentag(d) {
  const datum = berlinTeile(new Date(d)).slice(0, 10);
...
// Reihenfolge auf dem Platz, nicht im Alphabet: Alphabetisch käme
// ABW, ANG, MF, TW heraus – für einen Kader unbrauchbar.
export const POS_ORDNUNG = ["TW", "ABW", "MF", "ANG"];

export function posRang(p) {
  const kurz = Number.isFinite(Number(p)) ? POS[Number(p)] : String(p ?? "");
  const i = POS_ORDNUNG.indexOf(kurz);
  return i < 0 ? POS_ORDNUNG.length : i;
}
...
// ── Wer steht in der echten Aufstellung? ────────────────────────────
//
// Unter welchem Feld Kickbase die Startelf kennzeichnet, ist nicht belegt.
// Statt einen Feldnamen zu raten, wird gesucht — und zwar mit einer Probe,
// die sich selbst beweist: Gesucht ist ein Feld, bei dem **genau elf**
// Spieler des Kaders markiert sind. Bei einem Kader von 15 bis 20 Spielern
// ist das ein starkes Kennzeichen; trifft kein Feld genau elf, wird nichts
// zurückgegeben statt etwas Falsches.
const ELF = 11;

// Bekannte Kandidaten zuerst. Danach wird jedes andere Feld probiert —
// der Name ist ja gerade unbekannt.
const AUFSTELLUNG_FELDER = ["lo", "lineup", "lineupPosition", "lp", "st", "stx", "inLineup", "ins", "start"];

function markiert(wert) {
  if (wert === true) return true;
  if (wert === false || wert == null || wert === "") return false;
  const n = Number(wert);
  // Eine Position in der Aufstellung (1..11) zählt als markiert, ein
  // Marktwert in Millionen offensichtlich nicht.
  return Number.isFinite(n) && n >= 1 && n <= ELF;
}

// "Nicht aufgestellt" heißt: leer, false oder null — nicht "irgendeine
// andere Zahl".
function istAus(wert) {
  return wert === false || wert === 0 || wert === "0" || wert == null || wert === "";
}
...
export function findeAufstellung(rohListe) {
  ...
  if (liste.length <= ELF) return liste.map(() => true);
  ...
  const reihenfolge = [
    ...AUFSTELLUNG_FELDER.filter((f) => felder.has(f)),
    ...[...felder].filter((f) => !AUFSTELLUNG_FELDER.includes(f)),
  ];

  for (const f of reihenfolge) {
    const drin = liste.map((r) => markiert(r?.[f]));
    if (drin.filter(Boolean).length !== ELF) continue;

    // Gegenprobe: Die übrigen müssen auch erkennbar NICHT dabei sein.
    // Ohne sie ging ein Spieler-ID-Feld als Aufstellung durch — bei 18
    // Spielern liegen zufällig genau elf IDs zwischen 1 und 11.
    const restSauber = liste.every((r, i) => drin[i] || istAus(r?.[f]));
    if (restSauber) return drin;
  }
  return null;
}
