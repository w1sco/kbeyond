...
export function normalisiereSpieler(raw) {
  ...
  const nachname = pick(raw, ["ln", "pn", "n", "lastName", "name"]) ?? "";
  return {
    // `pi` zuerst: Das ist eindeutig die Spieler-ID. `i` kann je nach
    // Liste etwas anderes sein — bei einem Marktangebot etwa die ID des
    // Angebots. Wer die speichert, findet den Spieler nie wieder.
    id: pick(raw, ["pi", "i", "id"]),
