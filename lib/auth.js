...
import { zugangZuToken, FREI } from "./zugang.js";
...
export async function sitzung() {
  ...
  // Freigegeben? Geprüft wird am **Token**, nicht an einem Cookie mit der
  // Kennung — ein solches ließe sich fälschen, ein fremdes Token nicht.
  // Damit wirkt auch eine spätere Sperre sofort, statt bis zum Ablauf des
  // Tokens weiterzulaufen.
  const frei = await zugangZuToken(token);
  if (!frei || frei.status !== FREI) {
    redirect(`/login?zugang=${frei?.status ?? "unbekannt"}`);
  }
  ...
  return { token, uid, name, ablauf, nutzer: uid ?? name ?? "",
           kennung: frei.kennung, admin: frei.admin === true };
}
