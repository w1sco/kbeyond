...
// **Zeitfenster statt Schalter.** Vorher stand hier ein `boolean`. Der
// wurde nur von der Aktualisieren-Route zurückgesetzt — jeder andere
// Einstieg (also jeder Seitenaufruf) hat ihn nie angefasst. Eine warme
// Serverless-Instanz, die einmal in eine Drosselung gelaufen war, hat
// danach **jede** weitere Anfrage sofort abgebrochen: Die Ligaauswahl
// antwortete mit HTTP 500, obwohl Kickbase längst wieder lieferte.
//
// Mit einem Ablauf heilt sich das von selbst. Der laufende Lauf bricht
// weiterhin sofort ab — genau dafür ist die Sperre da —, aber eine
// Minute später darf es wieder jemand versuchen.
...
export function istGedrosselt() {
  return Date.now() < gedrosseltBis;
}

// Wie lange noch gesperrt ist – für die Meldung an den Nutzer.
export function sperreRestSekunden() {
  return Math.max(0, Math.ceil((gedrosseltBis - Date.now()) / 1000));
}
...
export async function kbFetch(path, token, versuch = 0) {
  if (istGedrosselt()) throw new GedrosseltFehler();
  ...
  if (res.status === 429 || res.status === 503) {
    ...
    if (versuch >= MAX_VERSUCHE) {
      // Ab jetzt bricht jeder weitere Aufruf sofort ab – aber nur für
      // die Dauer der Sperre, nicht bis zum Ende der Instanz.
      const dauer = Number.isFinite(sagt) && sagt > 0
        ? Math.min(sagt * 1000, 5 * 60_000)
        : SPERRE_MS;
      gedrosseltBis = Date.now() + dauer;
