...
export async function initSchema() {
  ...
  // Der Bundesliga-Spielplan mit Ergebnis je Seite. **Ligaunabhängig** —
  // die Bundesliga ist für alle dieselbe, und die Punkte einer Mannschaft
  // hängen nicht an der Kickbase-Liga.
  //
  // `punkte_heim`/`punkte_gast` sind die Kickbase-Punkte aller Spieler
  // dieser Mannschaft in diesem einen Spiel. Solange eine Partie nicht
  // gespielt oder nicht gewertet ist, stehen sie auf NULL — das
  // unterscheidet „kommt noch" von „0 Punkte geholt".
  await sql`
    CREATE TABLE IF NOT EXISTS spiele (
      ...
      PRIMARY KEY (spieltag, heim, gast)
    )`;
  await sql`CREATE INDEX IF NOT EXISTS spiele_tag ON spiele (spieltag)`;
  // Kickbases eigene Spiel-ID. Sie verbindet den Spielplan mit den
  // Einzelleistungen der Spieler — ohne sie wüsste man nicht, welches
  // Spiel gemeint ist.
  await sql`ALTER TABLE spiele ADD COLUMN IF NOT EXISTS mi TEXT`;
  await sql`ALTER TABLE spiele ADD COLUMN IF NOT EXISTS tore_heim INT`;
  await sql`ALTER TABLE spiele ADD COLUMN IF NOT EXISTS tore_gast INT`;
  await sql`CREATE UNIQUE INDEX IF NOT EXISTS spiele_mi ON spiele (mi)`;

  // Was ein Spieler in einem einzelnen Spiel geholt hat, und für wen.
  // **`team_id` ist der Verein zum Zeitpunkt des Spiels**, nicht der
  // heutige — sonst zählte ein Winterwechsel rückwirkend falsch.
  // Ligaunabhängig: Ein Bundesliga-Spiel ist für alle dasselbe.
  await sql`
    CREATE TABLE IF NOT EXISTS spieler_punkte (
      player_id TEXT NOT NULL,
      mi        TEXT NOT NULL,
      team_id   TEXT NOT NULL,
      spieltag  INT,
      punkte    INT,
      PRIMARY KEY (player_id, mi)
    )`;
  await sql`CREATE INDEX IF NOT EXISTS spieler_punkte_mi ON spieler_punkte (mi, team_id)`;

  // Wen wir schon nach seiner Leistungsreihe gefragt haben. Ein Aufruf je
  // Spieler ist teuer; ohne dieses Gedächtnis fragte jeder Lauf alle 470
  // erneut. `stand` sagt, bis zu welchem Spieltag der Spieler abgeholt ist.
  await sql`
    CREATE TABLE IF NOT EXISTS leistung_geprueft (
      player_id TEXT PRIMARY KEY,
      bis_tag   INT,
      geprueft  TIMESTAMPTZ DEFAULT NOW()
    )`;

  // Wie wahrscheinlich ein Spieler am kommenden Spieltag in der Startelf
  // steht (`prob` aus dem Spielerprofil, 1 = sicher bis 5 = spielt nicht).
  // Ligaunabhängig — die Aufstellung eines Vereins ist für alle dieselbe.
  //
  // `stufe` darf NULL sein: „gefragt, Kickbase sagt nichts" ist ein
  // Ergebnis und muss gespeichert werden, sonst kostet derselbe Spieler
  // bei jedem Lauf erneut einen Aufruf. `spieltag` sagt, für welchen
  // Spieltag die Angabe gilt — danach ist sie hinfällig.
  await sql`
    CREATE TABLE IF NOT EXISTS startelf (
      player_id TEXT PRIMARY KEY,
      stufe     INT,
      spieltag  INT,
      stand     TIMESTAMPTZ DEFAULT NOW()
  ...
  // Woher ein Tagesstand stammt: gemessen (beim Aktualisieren gespeichert)
  // oder aus den Events zurückgerechnet. Rekonstruierte Tage tragen keine
  // Punkte — die stehen nirgends und werden nicht geschätzt.
  await sql`ALTER TABLE tagesstand ADD COLUMN IF NOT EXISTS rekonstruiert BOOLEAN DEFAULT FALSE`;
  // Wie viele Spieler des Kaders an dem Tag einen bekannten Marktwert
  // hatten. Ohne diese Zahl ließe sich ein lückenhafter Kaderwert nicht
  // von einem echten unterscheiden.
  await sql`ALTER TABLE tagesstand ADD COLUMN IF NOT EXISTS mw_bekannt INT`;
  await sql`ALTER TABLE tagesstand ADD COLUMN IF NOT EXISTS mw_gesamt INT`;
  ...
  // Wie mit dem Liga-Admin umgegangen wird: automatisch (nur ausblenden,
  // wenn er nachweislich nicht mitspielt), immer zeigen, immer ausblenden.
  await sql`ALTER TABLE liga_settings ADD COLUMN IF NOT EXISTS admin_zeigen TEXT DEFAULT 'auto'`;
...
export async function getSettings(leagueId, nutzer = "") {
  ...
  await sql`
    INSERT INTO liga_settings (league_id, user_id, stichtag, startbudget, punkte_bonus, login_aktiv, login_start, spieltag_start, admin_zeigen, notiz)
...
// Der Spielplan mit allem, was bekannt ist. Ligaunabhängig.
export async function getSpiele() {
  const zeilen = await sql`
    SELECT spieltag, heim, gast, datum, punkte_heim, punkte_gast
    FROM spiele ORDER BY spieltag ASC, heim ASC`;
  return zeilen.map((z) => ({
    spieltag: Number(z.spieltag),
    heim: z.heim,
    gast: z.gast,
    datum: z.datum,
    // Nicht `?? null` auf 0 fallen lassen: Ein ungespieltes Spiel ist
    // etwas anderes als eines mit null Punkten.
    punkteHeim: z.punkte_heim == null ? null : Number(z.punkte_heim),
    punkteGast: z.punkte_gast == null ? null : Number(z.punkte_gast),
  }));
}

// Die Mannschaftspunkte je Spiel — aus den Einzelleistungen gerechnet,
// nicht gespeichert. So kann die Summe nie veralten, während weitere
// Spieler nachgeladen werden.
//
// **Gezählt wird nur, was gewertet ist** (Tore stehen fest). Vor dem
// Anpfiff stünde sonst überall eine 0, und eine 0 ist hier nicht von
// „noch nicht gespielt" zu unterscheiden.
// Die Startelf-Chancen als Nachschlagetabelle. Ein Spieler ohne Zeile und
// ein Spieler mit `stufe = NULL` sind dasselbe für die Anzeige: keine
// Angabe. Unterschieden werden sie nur beim Abrufen (gefragt oder nicht).
export async function getStartelf(spieltag = null) {
  const zeilen = spieltag == null
    ? await sql`SELECT player_id, stufe FROM startelf WHERE stufe IS NOT NULL`
    : await sql`SELECT player_id, stufe FROM startelf
                 WHERE stufe IS NOT NULL AND spieltag = ${spieltag}`;
  return new Map(zeilen.map((z) => [String(z.player_id), Number(z.stufe)]));
}

// Der nächste noch nicht gewertete Spieltag. Für den gilt jede
// Startelf-Prognose.
//
// Steht hier, nicht in startelfabruf.js: Der Pool-Aufbau und die
// Marktbeobachtung ernten `prob` nebenbei mit und brauchen die Zahl —
// über startelfabruf.js liefe das im Kreis (das importiert holePool).
...
// Geerntete Chancen wegschreiben. Ein Aufruf, egal wie viele Spieler.
//
// `spieltag` sagt, wofür die Angabe gilt. Ein bestehender Eintrag für
// **denselben** Spieltag wird überschrieben (der neuere Stand gewinnt);
// ein Eintrag aus einer früheren Woche sowieso.
export async function speichereChancen(eintraege, spieltag) {
  const liste = (eintraege ?? []).filter((e) => e?.id != null && e?.stufe != null);
  if (liste.length === 0 || spieltag == null) return 0;

  await sql`
    INSERT INTO startelf (player_id, stufe, spieltag, stand)
    SELECT t.player_id, t.stufe, ${spieltag}::int, NOW()
      FROM UNNEST(
        ${liste.map((e) => String(e.id))}::text[],
        ${liste.map((e) => Number(e.stufe))}::int[]
      ) AS t(player_id, stufe)
...
export async function getSpielePunkte() {
  const zeilen = await sql`
    SELECT s.spieltag, s.heim, s.gast, s.datum, s.tore_heim, s.tore_gast,
           s.tore_heim IS NOT NULL AND s.tore_gast IS NOT NULL AS gewertet,
           (SELECT SUM(p.punkte) FROM spieler_punkte p
             WHERE p.mi = s.mi AND p.team_id = s.heim) AS punkte_heim,
           (SELECT SUM(p.punkte) FROM spieler_punkte p
             WHERE p.mi = s.mi AND p.team_id = s.gast) AS punkte_gast,
           -- Wie viele Spieler dahinterstehen. Ohne diese Zahl liesse sich
           -- eine halb geladene Mannschaft nicht von einer schwachen
           -- unterscheiden — die Summe saehe in beiden Faellen echt aus.
           (SELECT COUNT(*) FROM spieler_punkte p
             WHERE p.mi = s.mi AND p.team_id = s.heim) AS spieler_heim,
           (SELECT COUNT(*) FROM spieler_punkte p
             WHERE p.mi = s.mi AND p.team_id = s.gast) AS spieler_gast
    FROM spiele s
    ORDER BY s.spieltag ASC, s.datum ASC NULLS LAST`;

  return zeilen.map((z) => {
    const gewertet = z.gewertet === true;
    return {
      spieltag: Number(z.spieltag),
      heim: z.heim,
      gast: z.gast,
      datum: z.datum,
      toreHeim: z.tore_heim == null ? null : Number(z.tore_heim),
      toreGast: z.tore_gast == null ? null : Number(z.tore_gast),
      // Punkte nur bei gewerteten Partien, und nur wenn wirklich schon
      // Leistungen geladen sind — sonst wäre die Null eine Behauptung.
      punkteHeim: gewertet && z.punkte_heim != null ? Number(z.punkte_heim) : null,
      punkteGast: gewertet && z.punkte_gast != null ? Number(z.punkte_gast) : null,
      spielerHeim: Number(z.spieler_heim ?? 0),
      spielerGast: Number(z.spieler_gast ?? 0),
    };
  });
}

// Der ganze Tagesverlauf einer Liga: alle vier Kennzahlen je Manager und
// Tag. Gemessene Tage und rekonstruierte stehen in derselben Tabelle und
// sind an `rekonstruiert` unterscheidbar.
export async function getTagesverlauf(leagueId) {
  const zeilen = await sql`
    SELECT manager_id, tag, teamwert, konto, punkte, rekonstruiert, mw_bekannt, mw_gesamt
    FROM tagesstand
    WHERE league_id = ${leagueId}
    ORDER BY tag ASC`;
  return zeilen.map((z) => ({
    managerId: String(z.manager_id),
    tag: z.tag,
    teamwert: z.teamwert == null ? null : Number(z.teamwert),
    konto: z.konto == null ? null : Number(z.konto),
    punkte: z.punkte == null ? null : Number(z.punkte),
    rekonstruiert: z.rekonstruiert === true,
    mwBekannt: z.mw_bekannt == null ? null : Number(z.mw_bekannt),
    mwGesamt: z.mw_gesamt == null ? null : Number(z.mw_gesamt),
  }));
}
