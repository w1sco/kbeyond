...
export async function initSchema() {
  ...
  // Punkteschnitt je Spiel (Kickbase `ap`) – kommt mit dem Kader, kostet
  // keinen Aufruf, und trägt die Seite „Aufgestellte Elf".
  await sql`ALTER TABLE kader ADD COLUMN IF NOT EXISTS schnitt NUMERIC`;
  ...
  // Die Tabelle `news` (Spielernachrichten) gab es einmal; die Seite ist
  // raus. Eine vorhandene Tabelle wird nicht gelöscht — ein DROP ist nicht
  // rückholbar, eine leere Tabelle kostet nichts. Sie wird nur nicht mehr
  // angelegt.
  ...
  // Hier standen einmal `spieler_punkte` und `leistung_geprueft` — die
  // Einzelleistungen je Spiel, ein Aufruf je Spieler. Sie trugen allein
  // die Gegner-Seite und sind mit ihr rausgeflogen. Vorhandene Tabellen
  // werden **nicht** gelöscht: Ein DROP ist nicht rückholbar, und leere
  // Tabellen kosten nichts.
...
export async function getKader(leagueId) {
  const zeilen = await sql`
    SELECT manager_id, player_id, name, position, marktwert, kaufpreis, punkte, schnitt,
           aufgestellt, stand
  ...
  for (const z of zeilen) {
    ...
    proManager.get(z.manager_id).push({
      ...
      punkte: z.punkte,
      schnitt: z.schnitt == null ? null : Number(z.schnitt),
...
// Der nächste Anpfiff, der noch bevorsteht — das erste Spiel des
// kommenden Spieltags. Daran hängt auf der Marktseite die Frage, ob ein
// Spieler noch rechtzeitig vorher vom Markt kommt.
export async function naechsterAnpfiff() {
  const r = await sql`SELECT MIN(datum) AS datum FROM spiele WHERE datum > NOW()`;
  return r[0]?.datum ? new Date(r[0].datum) : null;
}
