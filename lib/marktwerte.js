...
// Einmalige Sondierung: Welcher Kandidat liefert überhaupt etwas?
//
// Das hier ist der teure Teil, deshalb passiert er genau einmal und mit
// genau EINEM Spieler. Vorher wurden für jeden der 25 offenen Spieler bis
// zu zehn Kandidaten durchprobiert — 250 Anfragen, fast alles Fehlschläge.
// Genau das hat die Drosselung ausgelöst.
...
export async function ergaenzeMarktwerte(leagueId, token, opt = {}) {
  ...
  try {
    if (!pfad) {
      ...
      sondiert = true;
      if (!s.pfad) {
        // Kein Kandidat liefert etwas. Nicht weiter probieren.
        return { geholt: 0, offen: offen.length, sondiert, ohnePfad: true, gestoppt: false };
      }
      pfad = s.pfad;
      geholt++;
    }
