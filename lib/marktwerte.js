...
export async function ladeMarktwertVerlauf(leagueId, playerId, token) {
  ...
  if (reihe.length === 0) {
    // Nur sperren, wenn ein Endpunkt bekannt ist, der grundsätzlich
    // funktioniert — sonst liegt es an uns und nicht am Spieler.
    if (await bekannterPfad()) {
      await sql`
...
export async function ergaenzeMarktwerte(leagueId, token, opt = {}) {
  ...
  // Solange kein funktionierender Endpunkt bekannt ist, sind alte Sperren
  // wertlos — sie stammen aus einem Versuch, der nie klappen konnte. Weg
  // damit, sonst blockieren sie die Spieler auch nach einer Korrektur.
  if (!(await bekannterPfad())) {
